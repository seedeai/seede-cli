...
const uploadFileWithProgress = async ({ type, url, buffer, onProgress, ossMeta, contentType, filename = 'upload.bin' }) => {
    if (onProgress) onProgress({ phase: 'uploading', progress: 0 });
    if (type === 'oss') {
        const form = new FormData();
        form.append('success_action_status', '200');
        if (ossMeta?.policy) form.append('policy', ossMeta.policy);
        if (ossMeta?.signature) form.append('x-oss-signature', ossMeta.signature);
        form.append('x-oss-signature-version', ossMeta?.x_oss_signature_version || 'OSS4-HMAC-SHA256');
        if (ossMeta?.x_oss_credential) form.append('x-oss-credential', ossMeta.x_oss_credential);
        if (ossMeta?.x_oss_date) form.append('x-oss-date', ossMeta.x_oss_date);
        if (ossMeta?.key) form.append('key', ossMeta.key);
        if (ossMeta?.callback) form.append('callback', ossMeta.callback);
        if (ossMeta?.security_token) form.append('x-oss-security-token', ossMeta.security_token);
        const blob = new Blob([buffer], { type: contentType });
        form.append('file', blob, filename);
        const res = await fetch(ossMeta.host, {
            method: 'POST',
            body: form
        });
        if (!res.ok) {
        ...
        if (onProgress) onProgress({ phase: 'uploading', progress: 100 });
        return;
    } else {
